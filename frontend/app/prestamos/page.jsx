...
export default function Prestamos() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Header Responsivo */}
      <div className="bg-gradient-to-r from-[#003579] via-[#1f4c8b] to-[#2563eb] text-white px-3 py-5 sm:px-5 sm:py-7 lg:px-10 lg:py-9 shadow-lg">
        <div className="flex items-center space-x-3 sm:space-x-4">
          ...
          <div className="min-w-0 flex-1">
            <h1 className="text-xl sm:text-2xl lg:text-3xl font-bold tracking-tight truncate">Préstamos Entregados</h1>
      ...
      <div className="p-3 sm:p-5 lg:p-8">
        ...
        <div className="mb-5 sm:mb-7 space-y-3 sm:space-y-4">
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-10 pr-3 py-2.5 sm:py-3 bg-white rounded-lg sm:rounded-xl shadow-md border border-slate-200 focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent text-sm transition-all duration-200"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-2.5 sm:gap-3">
            ...
            <div className="flex gap-2 flex-wrap sm:flex-nowrap w-full">
              <button
                ...
                className={`px-3 py-2 text-xs sm:text-sm rounded-lg border transition-colors duration-200 flex-1 sm:flex-none whitespace-nowrap shadow-sm ${statusFilter === 'vencidas' ? 'bg-[#003579] text-white border-[#003579]' : 'bg-white text-[#003579] border-slate-200 hover:border-[#003579]/40 hover:shadow-md'}`}
              >
                Vencidas
              </button>
              <button
                onClick={() => setStatusFilter(statusFilter === 'proximas' ? '' : 'proximas')}
                className={`px-3 py-2 text-xs sm:text-sm rounded-lg border transition-colors duration-200 flex-1 sm:flex-none whitespace-nowrap shadow-sm ${statusFilter === 'proximas' ? 'bg-[#003579] text-white border-[#003579]' : 'bg-white text-[#003579] border-slate-200 hover:border-[#003579]/40 hover:shadow-md'}`}
              >
                Próximas a vencer
              </button>
            </div>
            ...
            <div className="flex gap-2">
              <select
                ...
                className="border rounded-lg px-3 py-2 text-[#003579] text-xs sm:text-sm flex-1 sm:min-w-[180px] bg-white focus:outline-none focus:ring-2 focus:ring-blue-300 shadow-sm transition-all duration-200"
              >
              ...
              <button
                ...
                className="px-3 py-2 text-xs sm:text-sm rounded-lg border bg-white text-[#003579] whitespace-nowrap hover:border-[#003579]/40 transition-colors duration-200 shadow-sm hover:shadow-md"
              >
                Limpiar
              </button>
            </div>
        ...
        {/* Grid de Cards con Skeleton Loaders para evitar saltos */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5 gap-3.5 sm:gap-5">
          {loading ? (
            // Skeleton Cards (muestra 8 placeholders para simular el layout)
            Array.from({ length: 8 }).map((_, index) => (
              <div
                key={`skeleton-${index}`}
                className="bg-white rounded-xl shadow-sm border border-slate-200/80 animate-pulse"
              >
            ...
            sorted.map((sol) => {
              ...
              return (
                <div
                  ...
                  className={`bg-white rounded-xl shadow-md cursor-pointer transition-all duration-200 border border-slate-200/80 hover:border-[#003579]/50 hover:shadow-lg active:scale-[0.98] ${overdue ? 'ring-2 ring-red-400/40 bg-red-50/50' : ''}`}
                >
                  <div className="p-4 sm:p-5">
                    <div className="flex items-center justify-between mb-3.5">
            ...
            <div className="bg-gradient-to-r from-[#003579] via-[#1f4c8b] to-[#2563eb] text-white px-4 sm:px-6 py-3">
              <div className="flex justify-between items-center">
                <div className="flex items-center space-x-2 sm:space-x-3 min-w-0 flex-1">
                  ...
                  <h2 className="text-base sm:text-lg font-bold truncate">Detalle del préstamo</h2>
                </div>
                <button
                  onClick={closeModal}
              ...
              {!detalle ? (
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5 sm:gap-5 mb-6 sm:mb-7">
                    <div className="bg-white rounded-xl p-4 sm:p-5 shadow-md border border-slate-100 transition-all duration-200 hover:shadow-lg">
                      <div className="flex items-center space-x-3 sm:space-x-4">
                        ...
                        <div className="min-w-0 flex-1">
                          <span className="block text-[11px] sm:text-xs text-slate-500 uppercase font-bold tracking-wide">Folio</span>
                    ...
                    <div className="bg-white rounded-xl p-4 sm:p-5 shadow-md border border-slate-100 transition-all duration-200 hover:shadow-lg">
                      <div className="flex items-center space-x-3 sm:space-x-4">
                        ...
                        <div className="min-w-0 flex-1">
                          <span className="block text-[11px] sm:text-xs text-slate-500 uppercase font-bold tracking-wide">Recolección</span>
                    ...
                    {detalle.nombre_alumno && (
                      <div className="bg-white rounded-xl p-4 sm:p-5 shadow-md border border-slate-100 transition-all duration-200 hover:shadow-lg">
                        <div className="flex items-center space-x-3 sm:space-x-4">
                          ...
                          <div className="min-w-0 flex-1">
                            <span className="block text-[11px] sm:text-xs text-slate-500 uppercase font-bold tracking-wide">Alumno</span>
                            <span className="block font-bold text-base sm:text-lg text-slate-800 break-words leading-snug">{detalle.nombre_alumno}</span>
                          </div>
                        </div>
                      </div>
                    )}
                    ...
                    {detalle.profesor && (
                      <div className="bg-white rounded-xl p-4 sm:p-5 shadow-md border border-slate-100 transition-all duration-200 hover:shadow-lg">
                        <div className="flex items-center space-x-3 sm:space-x-4">
                          ...
                          <div className="min-w-0 flex-1">
                            <span className="block text-[11px] sm:text-xs text-slate-500 uppercase font-bold tracking-wide">Profesor</span>
                            <span className="block font-bold text-base sm:text-lg text-slate-800 break-words leading-snug">{detalle.profesor}</span>
                          </div>
                        </div>
                      </div>
                    )}
                  ...
                  <form onSubmit={e => { e.preventDefault(); handleSave(); }}>
                    <div className="bg-white rounded-xl shadow-lg overflow-hidden border border-slate-200">
                      <div className="bg-slate-50 px-4 sm:px-6 py-3 sm:py-4 border-b border-slate-200">
                        <h3 className="text-base sm:text-lg font-bold text-slate-800 flex items-center space-x-3">
                      ...
                      <div className="overflow-x-auto">
                                      ...
                                      {reactivo ? (
                                        ...
                                        <div className="flex items-center justify-center gap-2">
                                          <input
                                            ...
                                            className="w-20 border border-slate-300 rounded-lg px-3 py-2 text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent shadow-sm"
                                          />
                                      ...
                                      <div className="flex items-center space-x-3">
                                        ...
                                        <div className="min-w-0 flex-1">
                                          <div className="flex items-center gap-2">
                                            ...
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[11px] font-semibold ${reactivo ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
                                      ...
                                      {reactivo ? (
                                        ...
                                        <div className="flex items-center justify-center gap-1.5">
                                          <input
                                            ...
                                            className="w-16 border border-slate-300 rounded-lg px-2 py-1 text-xs text-center focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent shadow-sm"
                                          />
                                      ...
                                      <div className="flex items-center space-x-2">
                                        ...
                                        <div className="min-w-0 flex-1">
                                          <div className="flex items-center gap-2">
                                            ...
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${reactivo ? 'bg-emerald-100 text-emerald-700' : 'bg-indigo-100 text-indigo-700'}`}>
                                              {reactivo ? 'Reactivo' : 'Material'}
                                            </span>
                                          </div>
                                        </div>
                        ...
                        <div className="block md:hidden space-y-4 p-2">
                          {detalle.items.map((item) => {
                            ...
                            const unidad = unidadPorTipo(item.tipo);
                            return (
                              <div key={item.item_id} className="bg-white rounded-xl p-4 border border-slate-200 shadow-md transition-shadow duration-200 hover:shadow-lg">
                                    ...
                                    {reactivo ? (
                                      ...
                                      <div className="flex items-center space-x-2">
                                        <input
                                          ...
                                          className="w-24 border border-slate-300 rounded-lg px-3 py-2 text-sm text-center focus:outline-none focus:ring-2 focus:ring-blue-300 focus:border-transparent shadow-sm"
                                        />
                    ...
                    <div className="flex flex-col sm:flex-row justify-end space-y-3 sm:space-y-0 sm:space-x-4 mt-6">
                      <button
                        type="button"
                        onClick={closeModal}
                        className="w-full sm:w-auto px-5 py-2.5 rounded-lg text-slate-700 hover:bg-slate-100 font-medium border border-slate-300 text-sm sm:text-base shadow-sm transition-all duration-200 hover:shadow-lg order-2 sm:order-1"
                      >
                        Cancelar
                      </button>
                      <button
